    ...
    return (
        <div id={"fablab-card-" + fabLab.id} className={"rounded-2xl h-61 bg-[#101010] flex flex-col overflow-hidden"}>
            ...
            <div className={"px-8 py-7"}>
                <h1 id={"fablab-card-" + fabLab.id + "-name"} className={"text-[20px] flex flex-row items-center gap-2 font-bold"}>{fabLab.name} <FabLabStateBadge fabLab={fabLab}/></h1>
                <p id={"fablab-card-" + fabLab.id + "-description"} className={"text-[13px] text-[#a0a0a0] mt-1 h-max-[32px] truncate"}>{fabLab.description}</p>
                <button
                    className={"h-10 bg-white mt-4 w-full text-[#101010] rounded-xl cursor-pointer font-medium"}
...
function FabLabStateBadge({ fabLab } : Readonly<{ fabLab: FabLab }>) {
    return (
        <span className={`text-[12px] w-16 h-5 flex flex-col items-center justify-center font-medium rounded-xl ${fabLab.active ? "bg-green-300 text-[#0a0a0a]" : "bg-red-400"}`}>
